...
```
- width : 뷰포터 너비 (device-width or 크기)
- height : 뷰포트 높이 (device-height or 크기)
- user-scalable: 확대/축소 가능여부(yes or no , yes : 기본값)
- initial-scale: 초기 확대 / 축소 값 (1~10 , 1 : 기본값)
- minimum-scale : 최소 확대 / 축소 값 (0~10 , 0.25: 기본값)
- maximum-scale : 최대 확대 / 축소값 (0~10 , 1.6 : 기본값)
...
@media screen and (min-width: 200px) and (min-width: 360px) /*추천 이것만 인지하자 07파트에서*/ @media screen and (max-width: 500px);
```

# FLEX(Felxible Box, Flexbox)

- CSS 레이아웃 모델로 화면 크기에 따라 레이아웃의 배치나 크기를 조절할떄 편리하게 사용
...
- Flex 켄테이너에 display:flex;를 적용하면서 시작
...
```css
.container {
  flex-direction: row or column oc row-reverse or column-reverse;
}
```
...
- flex-dorection 과 flex-wrap을 한꺼번에 지정할 수 있는 단축 속성
...
- stretch : 아이텥들을 위아래 방향으로 끝까지 늘림 (기본값)
...
```css
.container {
  align-content: flex-start or flex-end or center or space-between or
    space-around or space-evenly;
}
```
...
-flex-grow , flex-shrink , flex-basis를 한 번에 쑬 수 있는 속성
...
- 특정 아이템만 정렬을 따로 정려하고자 할 때 사용하는 속성
...
- Grid 켄테이너에 display:grid; 를 적용하면서 시작
