...
// 선택된 요소들 중 3번째 요소를 가져와서 스타일 변경
// eq (인덱스가 온다) -> 해당 텍스트 변경 ! 텍스트 변경 : text
...
console.log($("span").parant());
// 선택된 요소의 모든 상위 요소
console.log($("span").parants());

// span  요쇼의 부모 요소에 스타일 적용
// 글자색 - red, 테두리 : 2px solid

$("span").parant().css({ color: "red", border: "2px solid" });

// li 요소의 모든 상위 요소에 스타일 적용
// : 글자색: blue

$("li").not.css(color, "blue");

// li 요소의 상위 요소 중 div만 스타일 적용
// : 테두리  2px dashed tomato

// span 요소부터 div 요소 어전짜기 상위 요소 스타일 설정
$("li").parents("div").css("border", "2px dashed tomato");

// : 배경색 : lavenderblush
$("span").parentsUntill("div").css("background-color", "lavenderblush");

// 3. Descenandt
let style1 = { color: "deppink", border: "2px solid" };
...
// 자식의 자식의 자식 요소중 li 태그의 style4 적용
...
$(".wrap2 h2").nextUntill("p").css("border", "dashed");

// h2요소의 이전(prev) 형제 요소에 style5 적용
$(".wrap2 h2").prev().css(style5);

// h2요소의 이전에 오는 모든(prevAll) 형제 요소에
// 배경색 : moccasin
$(".wrap2 h2").prevAll().css("background-color", "moccasin");

// h2 요소 이전부터 p 요소 이후까지(prevUntill)의 모든 형제 요소에
// 글자크기 : 2rem
